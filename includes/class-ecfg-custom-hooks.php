...
class ECFG_Define_Custom_Hooks {
	...
	public function ecfg_e_date_function($start_date,$event_timezone) {
		$timezone =  $this->ecfg_e_timezone_function($event_timezone);
	...
	public function ecfg_e_title_function($event_title) {
	    ...
	    $title_tag = $this->template_function->ECFG_option_group_field('gc-advanced-settings','gc_event_desc_style','title_tag');
		$title_tag = isset($title_tag )?$title_tag : 'h4';
		$show_title = $this->template_function->ECFG_option_field('gc-event-attributes','gc-event-attribute-title');
	...
	public function ecfg_e_desc_function($event_content) {
	    ...
	    $show_desc  = $this->template_function->ECFG_option_field('gc-event-attributes','gc-event-attribute-description');
	...
	public function ecfg_e_location_function($event_location) {
	   	...
	   	$show_location = $this->template_function->ECFG_option_field('gc-event-attributes','gc-event-attribute-location');   
	...
	public function ecfg_e_time_function($start_date,$end_date,$alldayevent,$event_timezone) {
		...
		$show_time  = $this->template_function->ECFG_option_field('gc-event-attributes','gc-event-attribute-time');
		$show_time = isset($show_time) ? $show_time : 'on';
		$show_date  = $this->template_function->ECFG_option_field('gc-event-attributes','gc-event-attribute-date');
		...
		$event_start_time = date('H:i', strtotime($start_date));
		$event_end_date = date('Y-m-d', strtotime($end_date));
		$event_end_time = date('H:i', strtotime($end_date));
		$show_timezone  = $this->template_function->ECFG_option_field('gc-event-attributes','gc-event-attribute-timezome');
	...
	public function ecfg_e_timezone_function($event_timezone) {
		//gc-timezone-preference
		    $timezone_type = $this->template_function->ECFG_option_group_field('gc-advanced-settings','gc_event_timezone','gc_timezone_preference');
           	if ($timezone_type == 'custom')
			{
				$timezone = $this->template_function->ECFG_option_group_field('gc-advanced-settings','gc_event_timezone','gc_custom_timezone');
	...
	public function ecfg_e_more_function($event_link) {
	    ...
	    $show_readmore  = $this->template_function->ECFG_option_field('gc-event-attributes','gc-event-attribute-readmore');
